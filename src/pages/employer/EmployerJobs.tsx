...
const emptyForm = (): JobRequest => ({
  title: '', description: '', location: '', hourlyRate: undefined,
  hoursPerWeek: undefined, status: 'DRAFT', requiredSkills: [],
  workDays: [], shiftStartTime: '', shiftEndTime: '', maxApplicants: undefined,
});
...
const EmployerJobs: React.FC = () => {
  ...
  const openEdit = (job: Job) => {
    ...
    setForm({
      ...
      hourlyRate: job.hourlyRate, hoursPerWeek: job.hoursPerWeek,
      status: job.status, requiredSkills: job.requiredSkills || [],
      workDays: job.workDays || [], shiftStartTime: job.shiftStartTime || '',
      shiftEndTime: job.shiftEndTime || '', maxApplicants: job.maxApplicants,
    });
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { setMsg(err.response?.data?.message || 'Failed to save job.'); }
  };

  const handlePublish = async (id: number) => {
    try { await jobApi.publish(id); load(); } catch (e: any) { setMsg(e.response?.data?.message || 'Failed.'); }
  };
  const handleClose = async (id: number) => {
    try { await jobApi.close(id); load(); } catch (e: any) { setMsg(e.response?.data?.message || 'Failed.'); }
  };
  const handleDelete = async (id: number) => {
    if (!window.confirm('Delete this job?')) return;
    try { await jobApi.delete(id); load(); } catch (e: any) { setMsg('Failed to delete.'); }
  };
  ...
  return (
    <div>
      ...
      {loading ? <div className="loading">Loading...</div> : (
        <div className="card">
          {jobs.length === 0 ? (
            <div className="empty-state">No jobs yet. Create your first job posting!</div>
          ) : (
                    ...
                    <tr key={job.id}>
                      ...
                      <td>
                        <div className="flex-gap">
                          <button className="btn btn-outline btn-sm" onClick={() => openEdit(job)}>Edit</button>
                          {job.status === 'DRAFT' && <button className="btn btn-success btn-sm" onClick={() => handlePublish(job.id)}>Publish</button>}
                          {job.status === 'ACTIVE' && <button className="btn btn-warning btn-sm" onClick={() => handleClose(job.id)}>Close</button>}
                          {job.status === 'CLOSED' && <button className="btn btn-success btn-sm" onClick={() => handlePublish(job.id)}>Reopen</button>}
                          <Link to={`/employer/applications?jobId=${job.id}`} className="btn btn-secondary btn-sm">Applicants</Link>
      ...
      {/* Job Form Modal */}
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, overflowY: 'auto', padding: 16 }}>
          <div className="card" style={{ width: 600, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            <div className="card-title">{editing ? 'Edit Job' : 'Create New Job'}</div>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input type="number" value={form.hoursPerWeek || ''} onChange={e => setForm({ ...form, hoursPerWeek: Number(e.target.value) })} />
                </div>
                <div className="form-group">
                  <label>Max Applicants</label>
                  <input type="number" value={form.maxApplicants || ''} onChange={e => setForm({ ...form, maxApplicants: Number(e.target.value) })} />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <div className="flex-gap" style={{ marginBottom: 8 }}>
                  <input value={skillInput} placeholder="Add a skill"
              ...
              <div className="flex-gap">
                <button type="submit" className="btn btn-primary">{editing ? 'Update' : 'Create'} Job</button>
